...
class TPPlacementEngine:
    ...
    def validate_tp_placement(
        ...
        # ---------------------------------------------------------------
        # Law 4 — Crypto absolute veto (first check, no ATR needed)
        # ---------------------------------------------------------------
        if asset_class == AssetClass.CRYPTO:
            ...
            return TPValidationResult(
                ...
                rr_ratio=tp_dist / sl_distance if sl_distance > 0 else 0,
                atr_d1=None,
                ...
                time_stop_days=None,
                rejection_reason=(
                    "DIRECTIVE ZETA — LAW 4 ABSOLUTE VETO: Crypto instruments are excluded from "
                    "the CADES swing TP framework. No swing-style TP may be placed on crypto. "
                    "Close or restructure this position immediately."
                ),
            )
