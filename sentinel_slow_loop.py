...
def _handle_dead_market(symbol: str, atr: float) -> bool:
    """Handles the DEAD_MARKET short-circuit logic."""
    with _CYCLE_LOCK:
...
def _calculate_ml_consensus(symbol: str, df_ml: pd.DataFrame, df_m15: pd.DataFrame, is_this_symbol_starved: bool):
    """Orchestrates Kronos, XGB, and DDQN predictions, applies filters and returns consensus scores."""
    ...
    if os.path.exists(CHECKPOINT_PATH):
        try:
            ...
            ddqn_p_raw = ddqn_agent.infer_probability(feature_vec)
            if ddqn_p_raw is not None and not np.isnan(ddqn_p_raw):
                ddqn_p = ddqn_p_raw
...
def _run_episodic_memory_check(symbol: str, w_dist_norm: float):
    """Performs FAISS memory retrieval and calculates similarity/decay."""
...
def _prepare_local_meta_features(symbol, df_ml, x_prob, k_prob, max_sim, m_state, w_dist_norm, wasserstein_state, label_probs, volatility_ratio, sentiment_score):
    """Consolidates meta-features for the meta-model."""
    _final = df_ml.iloc[-1]
    def logit(p):
        p_clipped = np.clip(p, 1e-5, 1.0 - 1e-5)
        return np.log(p_clipped / (1.0 - p_clipped))
    sentiment_divergence_delta = abs(logit(sentiment_score) - logit(k_prob))
    local_meta_features = {
        "wasserstein_state": w_dist_norm, "hmm_state": wasserstein_state, "wasserstein_routing_probs": label_probs,
        "xgb_p": x_prob, "xgboost_prob": x_prob, "kronos_p": k_prob, "kronos_prob": k_prob,
        "faiss_sim": max_sim, "faiss_similarity": max_sim, "macro_sent": sentiment_score, "sentiment_score": sentiment_score,
        "sentiment_divergence_delta": sentiment_divergence_delta, "volatility_ratio": volatility_ratio,
        "macro_risk": float(m_state.get("black_swan_risk", 0.0)), "catalyst": float(m_state.get("asset_specific_catalysts", {}).get(symbol, 0.0)),
        "frac_diff": float(_final.get("frac_diff_price", 0.0)), "fft_amp_1": float(_final.get("fft_amp_1", 0.0)),
        "fft_amp_2": float(_final.get("fft_amp_2", 0.0)), "fft_amp_3": float(_final.get("fft_amp_3", 0.0)),
        "vpin": float(_final.get("vpin", 0.0)), "hawkes_intensity": float(_final.get("hawkes_intensity", 0.0)),
        "order_flow_entropy": float(_final.get("order_flow_entropy", 0.0)), "cs_rank": float(_GLOBAL_CS_RANKS.get(symbol, 0.5)),
        "uncertainty_width": float(m_state.get("uncertainty_width", 0.0)), "trust_gate_failed": False, "prediction_interval": [0.5, 0.5]
    }
    return local_meta_features

def _evaluate_meta_strategy(symbol, df_m15, df_ta, local_meta_features, label_probs, wasserstein_state, p_blend, primary_dir):
    """Determines active strategy and calculates meta_p."""
    ...
    return signal, meta_p, w_trend, w_range, p_trend, p_range

def _apply_dynamic_gate_corrections(symbol, df_ml, data_quality_flag, is_this_symbol_starved, w_trend, w_range, vrs):
    """Calculates entry gate with NY Open and volatility adjustments."""
    ...
    # NY Open Dynamic Gate Correction
    try:
    ...
    return current_gate, base_gate

def _check_hard_vetoes(symbol, meta_p, is_graveyard, is_legend, active_scores, label_probs, wasserstein_state, df_ml, latest_swing, atr, data_quality_flag):
    ...
    entropy_val = 0.0
    if df_ml is not None:
        try: entropy_val = float(df_ml.iloc[-1].get("order_flow_entropy", 0.0))
        except: pass
    if latest_swing is not None:
        try: entropy_val = max(entropy_val, float(latest_swing.get('entropy', 0.0)))
        except: pass
...
def run_inference_for_symbol(symbol: str, prep_data: dict):
    ...
    try:
        ...
        if data_quality_flag == "DEAD_MARKET":
            if _handle_dead_market(symbol, atr):
                return

        # ML Consensus
        try:
            ml_res = _calculate_ml_consensus(symbol, df_ml, df_m15, is_this_symbol_starved)
            p_blend = ml_res["p_blend"]; active_scores = ml_res["active_scores"]
            x_prob = ml_res["x_prob"]; k_prob = ml_res["k_prob"]
            ddqn_p = ml_res["ddqn_p"]; model_divergence = ml_res["model_divergence"]
            activity_ratio = ml_res["activity_ratio"]
        except Exception as e:
            logging.warning(f"[{symbol}] ML Bypass in effect. Reason: {e}")
            data_quality_flag = "DEGRADED"; x_prob = 0.500; ddqn_p = 0.500; k_prob = 0.500
            ml_res = {"activity_ratio": 1.0}; activity_ratio = 1.0; p_blend = 0.50
            active_scores = {"kronos": k_prob, "xgb": x_prob, "ddqn": ddqn_p}; model_divergence = 0.0
            if latest_swing is not None:
                rsi_v = latest_swing.get("rsi", 50); ent_v = latest_swing.get("entropy", 0.5)
                if rsi_v < 35 and ent_v > 0.85: p_blend = 0.85
                elif rsi_v > 65 and ent_v > 0.85: p_blend = 0.15
                elif latest_swing.get("trend_continuation_signal", 0) > 0: p_blend = 0.90 if df_m15["close"].iloc[-1] > latest_swing.get("ema_20", 0) else 0.10
                elif latest_swing.get("catalyst_momentum_signal", 0) > 0: p_blend = 0.80 if latest_swing.get("gap_pct", 0) > 0 else 0.20
                else: p_blend = 0.50
            else: p_blend = 0.50
        ...
        try:
            ...
            if "_LIVE_SWEEP_COUNTER" not in globals(): _LIVE_SWEEP_COUNTER = defaultdict(int)
            _LIVE_SWEEP_COUNTER[symbol] += 1
            if _LIVE_SWEEP_COUNTER[symbol] % 50 == 0 and df_ta is not None: _MATH_META_MODEL.optimize_hyperparameters(df_ta["close"], df_ta["WHL_vol"])
        except: pass

        # Wasserstein
        try:
            p_live = df_ml["frac_diff_price"].dropna().values[-500:] if len(df_ml["frac_diff_price"].dropna().values) > 500 else df_ml["frac_diff_price"].dropna().values
            p_train = df_ml["frac_diff_price"].dropna().values[:500] if len(df_ml["frac_diff_price"].dropna().values) >= 500 else np.random.normal(np.mean(p_live), np.std(p_live) + 1e-9, len(p_live))
            from scipy.stats import wasserstein_distance
            w_dist_norm = float(wasserstein_distance(np.sort(p_live), np.sort(p_train)) / (np.std(p_train) + 1e-9))
        except: w_dist_norm = 1.0

        if _OBSERVER:
            ts_i = _arctic_read(f"{symbol}_timesfm")
            p10 = float(ts_i.data.iloc[-1].get("p10", 0.0)) if ts_i is not None else 0.0
            p90 = float(ts_i.data.iloc[-1].get("p90", 0.0)) if ts_i is not None else 0.0
            _OBSERVER.observe(pd.DataFrame([{"kronos_p": k_prob, "xgb_p": x_prob, "ddqn_p": ddqn_p, "wasserstein_state": w_dist_norm, "atr": atr, "timesfm_p10": p10, "timesfm_p90": p90}]))

        primary_dir = 1 if p_blend > 0.60 else (-1 if p_blend < 0.40 else 0)
        is_legend, is_graveyard, max_sim = _run_episodic_memory_check(symbol, w_dist_norm)

        # Calibration update
        try:
            prev_meta_df = _arctic_read(f"{symbol}_meta")
            if prev_meta_df is not None and not prev_meta_df.data.empty and df_ta is not None and len(df_ta) >= 2:
                prev_row = prev_meta_df.data.iloc[-1]; prev_p = float(prev_row.get("meta_conviction", 0.5))
                price_change = float(df_ta["close"].iloc[-1]) - float(df_ta["close"].iloc[-2])
                if prev_p > 0.5: _MATH_META_MODEL.add_outcome(prev_p, 1.0 if price_change > 0 else 0.0)
                elif prev_p < 0.5: _MATH_META_MODEL.add_outcome(prev_p, 1.0 if price_change < 0 else 0.0)
        except: pass

        instant_atr_10 = calculate_atr_df(df_m15, 10); baseline_atr_200 = calculate_atr_df(df_m15, 200)
        vol_ratio = float(instant_atr_10 / baseline_atr_200) if baseline_atr_200 > 0.0 else 1.0
        ...
        local_meta_features = _prepare_local_meta_features(symbol, df_ml, x_prob, k_prob, max_sim, m_state, w_dist_norm, wasserstein_state, label_probs, vol_ratio, safe_sent)

        signal, meta_p, w_trend, w_range, p_trend, p_range = _evaluate_meta_strategy(symbol, df_m15, df_ta, local_meta_features, label_probs, wasserstein_state, p_blend, primary_dir)
        current_gate, base_gate = _apply_dynamic_gate_corrections(symbol, df_ml, data_quality_flag, is_this_symbol_starved, w_trend, w_range, vrs)
        ...
        if regime_prob < stagnant_regime_threshold and activity_ratio < stagnant_activity_threshold:
            ...
            _arctic_write(f"{symbol}_meta", pd.DataFrame([{"primary_dir": 0, "meta_conviction": 0.500, "wasserstein_state": "MARKET_STAGNANT", "hmm_state": "MARKET_STAGNANT", "strategy_type": "MOMENTUM", "atr": float(atr), "entropy": 0.0, "hawkes_intensity": 0.0, "timestamp": utils.get_utc_epoch(), "is_legend": False, "is_graveyard": False, "vpin": 0.5, "rsi": 50.0, "vrs": 1.0, "xgb_p": 0.500, "ddqn_p": 0.500, "volatility_ratio": vol_ratio}]))
            return
        elif regime_prob < 0.55: current_gate = max(current_gate - 0.05, 0.60)
        if is_graveyard: meta_p = 0.50
        if adjusted_conviction is not None and adjusted_conviction == 0.0: meta_p = 0.50

        logging.info(f"[{symbol}] MixTS BLEND: P={meta_p:.4f} (Gate: {current_gate:.3f})")
        ...
        if len(_P_SCORE_HISTORY) == 100:
            ...
            if p_std < 0.02:
                if _DRIFT_RECOVERY_ATTEMPTS < 3:
                    ...
                    logging.critical(f"[CRITICAL MODEL DRIFT] Mode Collapse ({p_std:.6f} < 0.02 limit). Resetting ({_DRIFT_RECOVERY_ATTEMPTS}/3)...")
                    ...
                    except Exception as e: logging.error(f"[DRIFT RECOVERY] Reset failed: {e}")
                else:
                    _MODEL_DRIFT_HALT = True
                    logging.critical(f"[CRITICAL MODEL DRIFT] Mode Collapse detected! std={p_std:.6f} < 0.02. Recovery attempts exhausted. HALTING.")
        with _CYCLE_LOCK: _CYCLE_P_SCORES[symbol] = float(meta_p)

        _arctic_write(f"{symbol}_meta", pd.DataFrame([{
            "primary_dir": int(1 if meta_p > 0.5 else (-1 if meta_p < 0.5 else 0)), "meta_conviction": float(meta_p), "wasserstein_state": w_dist_norm, "hmm_state": str(wasserstein_state),
            "strategy_type": signal["strategy_type"], "atr": float(atr), "timestamp": utils.get_utc_epoch(), "is_legend": is_legend, "is_graveyard": is_graveyard,
            "xgboost_prob": float(x_prob), "kronos_prob": float(k_prob), "faiss_similarity": float(max_sim), "sentiment_score": float(safe_sent), "volatility_ratio": vol_ratio,
            "entropy": float(df_ml.iloc[-1].get("order_flow_entropy", 0.0)), "hawkes_intensity": float(df_ml.iloc[-1].get("hawkes_intensity", 0.0)),
            "uncertainty_width": float(local_meta_features.get("uncertainty_width", 0.0)), "trust_gate_failed": bool(local_meta_features.get("trust_gate_failed", False)),
            "p_lower": float(local_meta_features.get("prediction_interval", [0.5, 0.5])[0]), "p_upper": float(local_meta_features.get("prediction_interval", [0.5, 0.5])[1]),
        }]))

        norm_p = abs(meta_p - 0.5) + 0.5
        if 0.40 <= meta_p <= 0.60:
            logging.info(f"[GATE] {symbol}: P={meta_p:.6f} in DEAD-ZONE.")
        elif norm_p >= current_gate and meta_p != 0.5:
            if not _check_hard_vetoes(symbol, meta_p, is_graveyard, is_legend, active_scores, label_probs, wasserstein_state, df_ml, latest_swing, atr, data_quality_flag):
                with _CYCLE_LOCK:
                    _CYCLE_PENDING_SIGNALS.append({
                        "symbol": symbol, "direction": "BUY" if meta_p > 0.5 else "SELL", "strategy_type": signal["strategy_type"], "conviction": round(float(meta_p), 6),
                        "sl": float(signal["sl"]), "tp": float(signal["tp"]), "size_multiplier": float(signal["size_multiplier"]), "tag": signal["tag"],
                        "xgb_p": float(x_prob), "ddqn_p": float(ddqn_p), "wasserstein_state": wasserstein_state, "atr": float(atr),
                        "timestamp": int(datetime.now(timezone.utc).timestamp()), "version": "v29.0-IRONCLAD-CADES", "signal_type": signal["strategy_type"],
                        "model_divergence": float(model_divergence), "vrs": float(vrs), "applied_dynamic_gate": float(current_gate)
                    })
