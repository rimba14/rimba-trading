...
def _check_sentinel_halt_conditions(symbol: str, force_refresh: bool = False) -> Optional[dict]:
    """
    Checks for various halt conditions including model drift, oracle cooldown,
    macro halts, and black swan events.
    Returns m_state dict if okay to proceed, otherwise returns None.
    """
...
def _ingest_multi_timeframe_data(symbol: str):
    """
    Ingests H1, H4, and M15/Tick data for a given symbol.
    Handles fallbacks for starved data and computes swing alpha.
    Returns (df_m15, df_h1, df_h4, swing_alpha, latest_swing, data_quality, is_starved) or None on fatal error.
    """
    ...
    if df_m15 is None or len(df_m15) < 512:
        ...
        try:
            ...
            return None
    return df_m15, df_h1, df_h4, swing_alpha, latest_swing, data_quality_flag, is_this_symbol_starved

def _handle_stagnation_check(symbol: str, df_m15: pd.DataFrame, df_h1: pd.DataFrame, is_starved: bool, m_state: dict) -> Optional[dict]:
    """
    Checks for zero-variance or market stagnation.
    Returns a result dict if stagnant, otherwise returns None.
    """
    if df_m15 is not None and not df_m15.empty:
        ...
        if pd.isna(price_variance) or price_variance < getattr(sentinel_config, 'STAGNANT_VARIANCE_THRESHOLD', 1e-7):
            ...
            return {
                ...
                "is_this_symbol_starved": is_starved, "atr": 0.0010, "m_state": m_state
            }
    return None

def _perform_feature_engineering(symbol: str, df_m15: pd.DataFrame, df_h1: pd.DataFrame, latest_swing: Any):
    """
    Performs TA and ML feature engineering.
    Returns (df_ml, df_ta, vrs, data_quality) or None on fatal error.
    """
    global _GLOBAL_CS_RANKS
    data_quality_flag = "PRISTINE"
    df_ta = df_m15.copy()
    ...
    try:
        ...
        if latest_swing is None:
            return None
    return df_ml, df_ta, vrs, data_quality_flag

def _analyze_market_regime(symbol: str, df_ml: pd.DataFrame, df_ta: pd.DataFrame, df_m15: pd.DataFrame, df_h1: pd.DataFrame, latest_swing: Any):
    """
    Analyzes market regime using Wasserstein clustering and HMM/BOCPD.
    Returns (wasserstein_state, wasser_prob, label_probs, atr, adjusted_conviction, cond_num).
    """
    ...
    return wasserstein_state, wasser_prob, label_probs, atr, adjusted_conviction, cond_num
...
def fetch_and_calculate_raw_features(symbol: str, force_refresh: bool = False) -> Optional[dict]:
    ...
    m_state = _check_sentinel_halt_conditions(symbol, force_refresh)
    if m_state is None:
        return None

    try:
        ingest_res = _ingest_multi_timeframe_data(symbol)
        if ingest_res is None:
            return None
        df_m15, df_h1, df_h4, swing_alpha, latest_swing, data_quality_flag, is_this_symbol_starved = ingest_res

        df_ta = df_ml = None

        stagnant_res = _handle_stagnation_check(symbol, df_m15, df_h1, is_this_symbol_starved, m_state)
        if stagnant_res:
            return stagnant_res

        feat_res = _perform_feature_engineering(symbol, df_m15, df_h1, latest_swing)
        if feat_res is None:
            return None
        df_ml, df_ta, vrs, data_quality_flag = feat_res
        ...
        regime_res = _analyze_market_regime(symbol, df_ml, df_ta, df_m15, df_h1, latest_swing)
        wasserstein_state, wasser_prob, label_probs, atr, adjusted_conviction, cond_num = regime_res
