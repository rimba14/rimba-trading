...
institutional_ledger = Arctic('lmdb://C:/Sentinel_Project/arctic_cache')
...
def get_meta_conviction(symbol, features, direction, base_p=0.5):
    """
    ...
    try:
        ...
        # Directive: SRE Surgical Amputation - Reverting to 6-D shape to match model on disk
        f_keys = ['W_rsi', 'W_macd', 'Wy_trend', 'B_bbpos', 'S_struct', 'WHL_vol']
        x = [features.get(k, 0.5) for k in f_keys]
        
        # 1. Base Probability (p_success) is now passed as an argument (p_blend)
        ...
        try:
            ...
            # Directive 2: Native MoE Reasoning (Phase 2)
            reasoning_data = _QWEN_ENGINE.json_with_retry(sys_prompt, user_prompt)
            logging.info(f"[{symbol}] MoE Reasoning: {reasoning_data.get('reasoning', 'N/A')}")
        except Exception as e:
            logging.error(f"Reasoning Engine Error for {symbol}: {e}")

        # Directive 2: Hybrid Conviction (Oracle Blend p * Reasoning Confidence)
        # Ensuring both the statistical (Kronos+XGB) and logical (MoE) gates agree.
        # Restore Fail-Safe Logic: Defaults to 0.50 (Neutral) if reasoning fails
        if "Max retries exceeded" in reasoning_data.get('reasoning', ''):
            p_final = 0.50
            logging.warning(f"[{symbol}] Reasoning Timed Out. FAIL-SAFE ENGAGED: Force Neutral Conviction.")
        else:
            reasoning_conf = reasoning_data.get('confidence', 0.5)
            p_final = (base_p * 0.6) + (reasoning_conf * 0.4)
...
# Configure Logging
log_file = r"C:\sentinel_logs\slow_loop_v17_production.log"
logging.basicConfig(
    level=logging.INFO, 
...
def update_slow_oracles(symbol: str):
    ...
    try:
        ...
        # Directive: Macro Halt Check (v17.0)
        HALT_PATH = "C:/Sentinel_Project/halt_signal.json"
        if os.path.exists(HALT_PATH):
            logging.critical(f"🛑 [MACRO_HALT] Global Suspension Active. Reason: See halt_signal.json")
            time.sleep(60)
            return
        ...
        meta_p = get_meta_conviction(symbol, current_features, primary_dir, base_p=p_blend)
...
def main():
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as warmup_executor:
